...
function addtocart($iddh, $idpro, $tensp, $img, $soluong, $dongia)
{
    $conn = connectdb();
    $sql = "INSERT INTO tbl_cart (iddh, idpro, tensp, img, soluong, dongia) 
    VALUES ('" . $iddh . "','" . $idpro . "','" . $tensp . "','" . $img . "','" . $soluong . "','" . $dongia . "')";
    // use exec() because no results are returned
    $conn->exec($sql);
    $last_id = $conn->lastInsertId();
    return $last_id;
